...
void lcd_init(void)
{
  ...
  {
    //Робота з watchdogs
    watchdog_routine(WATCHDOG_KYYBOARD, 1);
  ...
  *((unsigned char *) LCD_BASE) = 0x3A;
  ...
  {
    //Робота з watchdogs
    watchdog_routine(WATCHDOG_KYYBOARD, 2);
    new_count_tim4 = ((uint16_t) TIM4->CNT);
    if (new_count_tim4 >= current_count_tim4)
      delta = new_count_tim4 - current_count_tim4;
    else
  ...
  *((unsigned char *) LCD_BASE) = 0x3A;
  ...
  {
    //Робота з watchdogs
    watchdog_routine(WATCHDOG_KYYBOARD, 3);
    new_count_tim4 = ((uint16_t) TIM4->CNT);
    if (new_count_tim4 >= current_count_tim4)
      delta = new_count_tim4 - current_count_tim4;
    else
  ...
  *((unsigned char *) LCD_BASE) = 0x3A;
  ...
  //Робота з watchdogs
  watchdog_routine(WATCHDOG_KYYBOARD, 4);
  ...
  //Робота з watchdogs
  watchdog_routine(WATCHDOG_KYYBOARD, 5);
}
/*****************************************************/
...
unsigned int hd44780_puts_xy(unsigned char x, unsigned char y, unsigned char *s)
{
  ...
  if (error_LCD == 0)
    error_LCD = hd44780_puts(s, MAX_COL_LCD);
  if (error_LCD == 0)
  {
    error_LCD = hd44780_gotoxy(x, y);
    if (error_LCD == 0)
      error_LCD = hd44780_test(s, MAX_COL_LCD);
  }
...
void view_whole_ekran(void)
{
  if (current_ekran.current_action != ACTION_WITH_CARRENT_EKRANE_NONE)
  {
       ...
       (_CHECK_SET_BIT(set_diagnostyka, ERROR_LCD_BIT) == 0)))
    {
      ...
      if (error_LCD == 0)
        error_LCD = hd44780_gotoxy((current_ekran.position_cursor_x & (MAX_COL_LCD - 1)), (current_ekran.position_cursor_y & (MAX_ROW_LCD - 1)));
      //Виставляємо стан курсора: включено-виключено; мигає- не мигає
      if (error_LCD == 0)
        error_LCD = mode_viewing(1, current_ekran.cursor_on, current_ekran.cursor_blinking_on);
