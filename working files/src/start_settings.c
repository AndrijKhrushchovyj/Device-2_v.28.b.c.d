...
#error "UDEFINED MODIFIKACIA"
  ...
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4; /*30МГц/4 = 7.5 МГц*/
...
void min_settings(__SETTINGS *target_label)
{
  ...
  target_label->control_Umin = CONTROL_UMIN_WORK;
  ...
  for (unsigned int i = 0; i < NUMBER_GROUP_USTAVOK; i++)
  {
    ...
    target_label->timeout_Umin1[i] = TIMEOUT_UMIN1_WORK;
    target_label->timeout_Umin2[i] = TIMEOUT_UMIN2_WORK;
