...
void make_ekran_setpoint_rpn(unsigned int group)
{
  ...
  unsigned int vaga, value1 = 0, value2, first_symbol;
    ...
    if (index_of_ekran_tmp < (unsigned int) (MAX_ROW_FOR_SETPOINT_RPN))
    {
      if ((i & 0x1) == 0)
      {
        ...
        if (index_of_ekran_tmp == INDEX_ML_STPRPN_OSN)
        {
          ...
          if (view == true)
            value1 = current_settings.setpoint_rpn_osn[group]; //у змінну value1 поміщаємо значення уставки
          else
            value1 = edition_settings.setpoint_rpn_osn[group];
        }
        else if (index_of_ekran_tmp == INDEX_ML_STPRPN_ZONE)
        {
          vaga = 100; //максимальний ваговий коефіцієнт для вилілення старшого розряду
          if (view == true)
            value1 = current_settings.setpoint_rpn_zony[group]; //у змінну value1 поміщаємо значення уставки
          else
            value1 = edition_settings.setpoint_rpn_zony[group];
        }
        else if (index_of_ekran_tmp == INDEX_ML_STPRPN_MAX_PER)
        {
          vaga = 10; //максимальний ваговий коефіцієнт для вилілення старшого розряду
          if (view == true)
            value1 = current_settings.setpoint_rpn_per[group]; //у змінну value1 поміщаємо значення уставки
          else
            value1 = edition_settings.setpoint_rpn_per[group];
        }
        else if (index_of_ekran_tmp == INDEX_ML_STPRPN_DOD)
        {
          vaga = 100000; //максимальний ваговий коефіцієнт для вилілення старшого розряду
          if (view == true)
            value1 = current_settings.setpoint_rpn_dod[group]; //у змінну value1 поміщаємо значення уставки
          else
            value1 = edition_settings.setpoint_rpn_dod[group];
        }
        else if (index_of_ekran_tmp == INDEX_ML_STPRPN_K)
        {
          vaga = 100; //максимальний ваговий коефіцієнт для вилілення старшого розряду
          if (view == true)
            value1 = current_settings.setpoint_rpn_K[group]; //у змінну value1 поміщаємо значення уставки
          else
            value1 = edition_settings.setpoint_rpn_K[group];
        }

        first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений

        //Робимо копію, яку можга буде потім редагувати
        value2 = value1;
      }
      else
      {
        ...
        for (unsigned int j = 0; j < MAX_COL_LCD; j++)
        {
          if (index_of_ekran_tmp == INDEX_ML_STPRPN_OSN)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value2, &vaga, &first_symbol, j, COL_SETPOINT_RPN_OSN_COMMA, view, 0);
          }
          else if (index_of_ekran_tmp == INDEX_ML_STPRPN_ZONE)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value2, &vaga, &first_symbol, j, COL_SETPOINT_RPN_ZONE_COMMA, view, 0);
          }
          else if (index_of_ekran_tmp == INDEX_ML_STPRPN_MAX_PER)
          {
            if ((value1 == SETPOINT_RPN_PER_UNLIMITED) && (current_ekran.edition == 0))
            {
            ...
            else
            {
              if ((j < COL_SETPOINT_RPN_MAX_PER_BEGIN) || (j > COL_SETPOINT_RPN_MAX_PER_END))
                ...
                calc_int_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value2, &vaga, &first_symbol, view);
            }
          }
          else if (index_of_ekran_tmp == INDEX_ML_STPRPN_DOD)
          {
            if (
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value2, &vaga, &first_symbol, j, COL_SETPOINT_RPN_DOD_COMMA, view, 0);
          }
          else if (index_of_ekran_tmp == INDEX_ML_STPRPN_K)
          {
            if ((j < COL_SETPOINT_RPN_K_BEGIN) || (j > COL_SETPOINT_RPN_K_END))
              ...
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value2, &vaga, &first_symbol, j, COL_SETPOINT_RPN_K_COMMA, view, 0);
          }
        }
      }
    }
  ...
  if (current_ekran.edition == 0)
  {
    ...
    {
      if ((value1 >= SETPOINT_RPN_PER_MIN) && (value1 <= SETPOINT_RPN_PER_MAX))
      {
