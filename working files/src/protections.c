...
inline void up_handler(unsigned int *p_active_functions, unsigned int number_group_stp)
{
  for (size_t n_UP = 0; n_UP < NUMBER_UP; n_UP++)
  {
    ...
    static uint32_t const none_int32_t = MASKA_FOR_BIT(8 * sizeof(int32_t) - 1);
...
inline int control_pologennja_part1(unsigned int *p_active_functions)
{
  ...
  uint32_t const low_trtrad =
    ...
    (_CHECK_SET_BIT(p_active_functions, RANG_BCD_2_BIT) != 0) * 4 +
    (_CHECK_SET_BIT(p_active_functions, RANG_BCD_3_BIT) != 0) * 8;
...
#undef N_SOURCE
  ...
  if (semaphore_measure_values_low == 0)
  {
    ...
    frequency_val_1_middle = frequency_val_1;
    frequency_val_2_middle = frequency_val_2;
  }
  else
  {
    for (unsigned int i = 0; i < _NUMBER_IM; i++)
