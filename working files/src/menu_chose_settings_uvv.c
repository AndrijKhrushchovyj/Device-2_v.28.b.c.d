  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    ...
    if (index_of_ekran_tmp < NUMBER_INPUTS)
    {
      if ((i & 0x1) == 0)
      {
        ...
        if (view == true)
          value = current_settings.dopusk_dv[index_of_ekran_tmp]; //у змінну value поміщаємо значення допуску дискретного входу
        else
          value = edition_settings.dopusk_dv[index_of_ekran_tmp];
    ...
    if (index_of_ekran < (NUMBER_INPUTS << 1)) //Множення на два константи NUMBER_INPUTS потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)
    {
      ...
      else
      {
        ...
        if (type_input_or_signal == 0)
        {
          ...
          if (current_ekran.edition == 0)
            temp_data = current_settings.type_of_input;
          else
            temp_data = edition_settings.type_of_input;

          for (unsigned int j = 0; j < MAX_COL_LCD; j++)
            working_ekran[i][j] = information[index_language][(temp_data >> index_ctr) & 0x1][j];
          if (position_temp == index_ctr)
        ...
        else
        {
          ...
          if (current_ekran.edition == 0)
            temp_data = current_settings.type_of_input_signal;
          else
            temp_data = edition_settings.type_of_input_signal;
  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    if (index_of_ekran < (NUMBER_SIMPLE_OUTPUTS << 1)) //Множення на два константи NUMBER_SIMPLE_OUTPUTS потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)
    {
      ...
      else
      {
        ...
        if (value == true)
          value += ((*p_temp_data_modif & maska) != 0); //тільки у випадку, коли вихід сигнальний
  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    if (index_of_ekran < (NUMBER_LEDS << 1)) //Множення на два константи NUMBER_LEDS потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)
    {
      ...
      else
      {
        ...
        if (current_ekran.edition == 0)
          temp_data = current_settings.type_of_led;
        else
          temp_data = edition_settings.type_of_led;
  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    if (index_of_ekran < (NUMBER_DEFINED_BUTTONS << 1)) //Множення на два константи NUMBER_BUTTON потрібне для того, бо на одну позицію ми використовуємо два рядки (назва + значення)
    {
      if ((i & 0x1) == 0)
      {
        ...
        for (unsigned int j = 0; j < MAX_COL_LCD; j++)
        {
          if (j != first_index_number)
            working_ekran[i][j] = name_string[j];
          else
            working_ekran[i][j] = ((index_of_ekran >> 1) + 1) + 0x30;
        }
      }
      else
      {
