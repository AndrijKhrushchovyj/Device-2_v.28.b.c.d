...
void make_ekran_current(unsigned int pervynna_vtorynna)
{
  ...
  for (unsigned int i = 0; i < MAX_ROW_FOR_MEASURMENT_CURRENT; i++)
  {
    ...
    {
      if (index_language == INDEX_LANGUAGE_EN) name_string_tmp[i][4] = 'c';
      else name_string_tmp[i][4] = 'р';
  ...
  /******************************************/
  {
    ...
    {
      if (index_array[i + additional_current] == delete_meas) break;
    }
//    int i = delete_index - additional_current;
    ...
    do
    {
      ...
      {
        if ((i + 1) < (MAX_ROW_FOR_MEASURMENT_CURRENT - additional_current)) name_string_tmp[i][j] = name_string_tmp[i + 1][j];
...
void make_ekran_voltage_phase(unsigned int pervynna_vtorynna)
{
  ...
  if ((current_settings.control_extra_settings_1 & MASKA_FOR_BIT(INDEX_ML_CTREXTRA_SETTINGS_1_CTRL_PHASE_LINE)) == 0)
  {
    ...
    {
      for (unsigned int j = 0; j < MAX_COL_LCD; j++) name_string[row][j] = name_phase[index_language][i][j];
  ...
  for (unsigned int j = 0; j < MAX_COL_LCD; j++) name_string[row][j] = name_3U0[index_language][j];
    ...
    if ((current_settings.control_extra_settings_1 & MASKA_FOR_BIT(INDEX_ML_CTREXTRA_SETTINGS_1_CTRL_PHASE_LINE)) != 0)
    {
      for (__full_ort_index index_tmp = FULL_ORT_Ua; index_tmp <= FULL_ORT_Uc; index_tmp++)
      {
        ...
        do
        {
          for(unsigned int j = 0; j < MAX_COL_LCD; j++)
          {
            if ((i+1) < (MAX_ROW_FOR_MEASURMENT_ANGLE - additional_current)) name_string_tmp[i][j] = name_string_tmp[i + 1][j];
            else name_string_tmp[i][j] = ' ';
          }
