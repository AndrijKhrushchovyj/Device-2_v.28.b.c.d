  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    ...
    if (index_of_ekran_tmp < MAX_ROW_FOR_SETPOINT_Umax)
    {
      if ((i & 0x1) == 0)
      {
        ...
        if (index_of_ekran_tmp == INDEX_ML_STPUmax_Umax1)
        {
          vaga = 100000 / 1000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для уставки "Уставка ЗНмакс1"
          if (view == true)
            value = current_settings.setpoint_Umax1[group]; //у змінну value поміщаємо значення уставки Umax1
          else
            value = edition_settings.setpoint_Umax1[group];

          value /= 1000; //Відображаємо тільки ціоу частину, а витримка у пам'яті зберігається з точністю до мілісекунд
        }
        else if (index_of_ekran_tmp == INDEX_ML_STPUmax_Umax2)
        {
          vaga = 100000 / 1000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для уставки "Уставка ЗНмакс2"
          if (view == true)
            value = current_settings.setpoint_Umax2[group]; //у змінну value поміщаємо значення уставки Umax2
          else
            value = edition_settings.setpoint_Umax2[group];
      ...
      else
      {
        ...
        for (unsigned int j = 0; j < MAX_COL_LCD; j++)
        {
          ...
          {
            if (
              ((j < COL_SETPOINT_Umax_Umax1_BEGIN) || (j > COL_SETPOINT_Umax_Umax1_END)) &&
              (j != (COL_SETPOINT_Umax_Umax1_END + 2)))
              working_ekran[i][j] = ' ';
            else if (j == (COL_SETPOINT_Umax_Umax1_END + 2))
          ...
          {
            if (
              ((j < COL_SETPOINT_Umax_Umax2_BEGIN) || (j > COL_SETPOINT_Umax_Umax2_END)) &&
              (j != (COL_SETPOINT_Umax_Umax2_END + 2)))
              working_ekran[i][j] = ' ';
            else if (j == (COL_SETPOINT_Umax_Umax2_END + 2))
              working_ekran[i][j] = odynyci_vymirjuvannja[index_language][INDEX_V];
            else
              calc_int_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, view);
          }
...
void make_ekran_timeout_Umax(unsigned int group)
{
  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    ...
    if (index_of_ekran_tmp < MAX_ROW_FOR_TIMEOUT_UMAX)
    {
      if ((i & 0x1) == 0)
      {
        ...
        for (unsigned int j = 0; j < MAX_COL_LCD; j++)
          working_ekran[i][j] = name_string[index_language][index_of_ekran_tmp][j];
        if (index_of_ekran_tmp == INDEX_ML_TMOUmax_Umax2)
        {
          vaga = 10000; //максимальний ваговий коефіцієнт для вилілення старшого розряду для витримки Umax1
          if (view == true)
            value = current_settings.timeout_Umax2[group]; //у змінну value поміщаємо значення витримки Umax1
          else
            value = edition_settings.timeout_Umax2[group];
        }
        first_symbol = 0; //помічаємо, що ще ніодин значущий символ не виведений
      }
      else
      {
        ...
        for (unsigned int j = 0; j < MAX_COL_LCD; j++)
        {
          ...
          {
            if (
              ((j < COL_TMO_Umax_Umax2_BEGIN) || (j > COL_TMO_Umax_Umax2_END)) &&
              (j != (COL_TMO_Umax_Umax2_END + 2)))
              working_ekran[i][j] = ' ';
            else if (j == COL_TMO_Umax_Umax2_COMMA)
              working_ekran[i][j] = ',';
            else if (j == (COL_TMO_Umax_Umax2_END + 2))
              working_ekran[i][j] = odynyci_vymirjuvannja[index_language][INDEX_SECOND];
            else
              calc_symbol_and_put_into_working_ekran((working_ekran[i] + j), &value, &vaga, &first_symbol, j, COL_TMO_Umax_Umax2_COMMA, view, 0);
          }
        }
      }
    }
  ...
  if (current_ekran.edition == 0)
  {
    ...
    if (current_ekran.index_position == INDEX_ML_TMOUmax_Umax2)
    {
      ...
      last_position_cursor_x = COL_TMO_Umax_Umax2_END;
    }
  ...
  for (unsigned int i = 0; i < MAX_ROW_LCD; i++)
  {
    ...
    if (index_of_ekran_tmp < MAX_ROW_FOR_CONTROL_UMAX)
    {
      ...
      else
      {
        ...
        if (current_ekran.edition == 0)
          temp_data = current_settings.control_Umax;
        else
          temp_data = edition_settings.control_Umax;
