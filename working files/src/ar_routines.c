...
void ar_routine_with_fatfs(unsigned int before_full_start)
{
    ...
    (_CHECK_SET_BIT(set_diagnostyka, ERROR_FATFS) == 0))
  {
    ...
    if (
      ...
      ((state_ar_record_fatfs == STATE_AR_NONE_FATFS) ||
       (state_ar_record_fatfs == STATE_AR_MEMORY_FULL_FATFS) ||
       (state_ar_record_fatfs == STATE_AR_BLOCK_FATFS)) &&
      ((control_tasks_dataflash & (TASK_MAMORY_PART_PAGE_PROGRAM_THROUGH_BUFFER_DATAFLASH_FOR_FS |
                                   TASK_MAMORY_PAGE_PROGRAM_THROUGH_BUFFER_DATAFLASH_FOR_FS |
                                   TASK_MAMORY_READ_DATAFLASH_FOR_FS)) == 0))
    {
  ...
  if (state_ar_record_fatfs != STATE_AR_MEMORY_FULL_FATFS)
    _SET_BIT(clear_diagnostyka, ERROR_AR_MEMORY_FULL_BIT);
  /***/
    ...
    case STATE_AR_WRITE_FATFS:
      {
        switch (level_ar_writing)
        {
          ...
          case LEVEL_AR_WRITING_HEADER:
            {
              ...
              if (arDateTimeState == AVAT_DATE_TIMR_FIX_LEVEL1)
          ...
          case LEVEL_AR_WRITING_DATA:
            ...
            ***/
              ...
              else if (index_array_ar_tail == index_array_ar_heat)
              {
                state_ar_record_fatfs = STATE_AR_WAIT_TO_STOP_WRITE_FATFS;
              }
           ...
           (dirty_data_ar != false)))
        {
            ...
            (dirty_data_ar != false))
          {
            ...
            if (f_size(&fil) > sizeof(__HEADER_AR))
              res = f_sync(&fil);
            else
            {
              res = f_close(&fil);
              if (res == FR_OK)
              {
                  ...
                  (res == FR_NO_FILE))
                {
                  if (last_number > first_number)
                  {
                    first_number -= 1;
                    if (first_number < 0)
                      first_number += NUMBER_FATFS_NAME;
                  }
