...
__control_info const control_info[] =
{
	...
	{&current_settings_prt.control_mtz04, N_BIT_CTRMTZ04_1},
	{&current_settings_prt.control_mtz04, N_BIT_CTRMTZ04_2},
...
//    while ((error_status & (USART_FLAG_IDLE | USART_FLAG_LBD)) == 0);
     ...
     )   
  {
       ...
       )   
    {
      ...
      uint8_t sum = 0;
      for (int32_t i = 0; i < (size_packet - 3); i++) sum += (*pCanal1_MO_Received)[1 + i];
        ...
        case SENDING_GOOSE_INFO:
          {
            ...
            if ((*pCanal1_MO_Received)[index++] != 0)
            {
              ...
              if (index_tmp > 0)
              {
                for(size_t i = 0; i < sizeof(time_t); i++)  *((uint8_t *)(&time_dat_save_h) + i) = (*pCanal1_MO_Received)[index_tmp++];
                int32_t time_ms_tmp = 0;
                for(size_t i = 0; i < sizeof(int32_t); i++)  *((uint8_t *)(&time_ms_tmp) + i) = (*pCanal1_MO_Received)[index_tmp++];
